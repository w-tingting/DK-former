...
def quantization_model(model):
    for name, mod in model.named_children():
        if isinstance(mod, (Conv_RFF, Conv_Linear)):
            ...
            groups = mod.group
            F0 = mod.F0
            if isinstance(mod, Conv_Linear):
                W_mean = mod.W_mean
                W_logsigma = mod.W_logsigma
                W_eps = mod.W_eps

                W_from_q = (W_mean + torch.exp(W_logsigma / 2.) * W_eps)

                W_from_q = W_from_q.view(mc, kernel_size, kernel_size, in_channels, out_channels)
                W_from_q = W_from_q.permute(0, 4, 3, 1, 2)
                W_from_q = W_from_q.reshape(out_channels * mc, int(in_channels), kernel_size, kernel_size)

                setattr(model, name,
                        QConv_Linear(in_channels, out_channels, kernel_size, stride, padding, mc, groups, F0))
                getattr(model, name).conv2d.weight.data = W_from_q

            elif isinstance(mod, Conv_RFF):
                kernel_type = mod.kernel_type

                theta_logsigma = mod.theta_logsigma

                Omega_mean = mod.Omega_mean
                Omega_logsigma = mod.Omega_logsigma
                Omega_eps = mod.Omega_eps

                Omega_from_q = Omega_eps * torch.exp(Omega_logsigma / 2) + Omega_mean
                Omega_from_q = Omega_from_q.view(mc, kernel_size, kernel_size, in_channels, out_channels)
                Omega_from_q = Omega_from_q.permute(0, 4, 3, 1, 2)
                Omega_from_q = Omega_from_q.reshape(out_channels * mc, int(in_channels), kernel_size, kernel_size)

                setattr(model, name,
                        QConv_RFF(theta_logsigma, in_channels, out_channels, kernel_size, stride, padding, mc,
                                  kernel_type, groups, F0))
                getattr(model, name).conv2d.weight.data = Omega_from_q

        elif isinstance(mod, FullyConnected):
            ...
            setattr(model, name, QLinear(in_features, out, mc))
            for i in range(mc):
                getattr(model, name).linears[i].weight.data = W_from_q[i].t()
